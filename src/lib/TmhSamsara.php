...
//    }
    ...
    public function nodes(array $nodes): string
    {
        ...
        foreach ($nodes as $node) {
            ...
            } elseif ($node['htmlTag'] == 'book_group') {
                $html .= $this->bookGroupNode($node['attributes']);
            } else {
    ...
    public function bookGroupNode(array $attributes): string
    {
        $books = $this->filterEntity('book', $attributes['filters']);
        $bookRoutes = [];
        foreach ($books as $primaryKey => $book) {
            $bookRoutes[] = $this->findRoute($primaryKey);
        }
        $childNodes = [];
        foreach ($bookRoutes as $bookRoute) {
            $childNodes[] = [
                'htmlTag' => 'a',
                'attributes' => [
                ...
                'childNodes' => [],
                'innerHTML' => str_replace('_', ' ', $bookRoute['innerHtml']),
                'selfClosing' => false
            ];
    ...
    public function routeNode(array $attributes): string
    {
        ...
        if (in_array('innerHTML', array_keys($attributes))) {
            $locale = $this->currentLocale();
            $innerHtml = $this->translate($attributes['innerHTML'], $locale);
        }
        $node = [
            'htmlTag' => 'a',
            'attributes' => [
